...
if __name__ == '__main__':
    ...
    with tf.device(device):
        ...
        clipped_v2 = np.clip(undo_image_avg(image_original[0,:,:,:]+v2[0,:,:,:]), 0, 255) - np.clip(undo_image_avg(image_original[0,:,:,:]), 0, 255)

        image_perturbed = image_original + clipped_v2[None, :, :, :]
        ...
        plt.subplot(1, 2, 2)
        #imask = image_perturbed < 1  
        #image_perturbed[mask]=50  
        plt.imshow(undo_image_avg(image_perturbed[0, :, :, :]).astype(dtype='uint8'),interpolation=None)
        plt.title(str_label_perturbed)
        plt.savefig('13_sp.png')


        plt.subplot(1,2,1)
        mask2 = image_perturbed < 12 
        image_perturbed[mask2]=120
        plt.imshow(undo_image_avg(image_perturbed[0, :, :, :]).astype(dtype='uint8'),interpolation=None)
        plt.title(str_label_original)
        plt.savefig('13_lp.png')
        

        plt.subplot(1,2,2)
        mask3 = image_perturbed < 87 
        image_perturbed[mask3]=255
        plt.imshow(undo_image_avg(image_perturbed[0, :, :, :]).astype(dtype='uint8'),interpolation=None,cmap='brg')
        plt.savefig('13_lp2.png')
        ...
        #edges = filters.sobel(image_perturbed[0, :, :, :]).astype(dtype='uint8')  
       # plt.imshow(edges, cmap='gray')
        #plt.savefig('final34.png')ccent
        #plt.show()      
          
        from PIL import Image,ImageFilter  
        #Read image
        im2 = Image.open('data/test_img.png')
        #Display image  
       #   im2.show()
